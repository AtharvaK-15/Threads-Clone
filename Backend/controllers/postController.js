...
const createPost = async (req, res) => {
    try {
        ...
        if(user._id.toString() !== req.user._id.toString()) return res.status(401).json({error:"Unauthorized!"}); //check if the user who is posting is the same as the user who is logged in
...
const deletePost = async (req, res) => {
    try {
        ...
        if(post.postedBy.toString() !== req.user._id.toString()) return res.status(401).json({error:"Unauthorized!"}); //check if the user who is deleting is the same as the user who is logged in
